...
aka Retea Neuronala FeedForward
'''
...
mlp = MLPClassifier(hidden_layer_sizes=(3500,), activation='logistic', solver='adam', alpha=0.001, max_iter=100000) # set the method
start_time = time.time()
mlp.fit(all_data_train_bow[indices_train_valid, :], labels[indices_train_valid])
print("End time")
elapsed_time = time.time() - start_time
print("elapsed time ", elapsed_time)
predictions = mlp.predict(all_data_train_bow[indices_test])
print("MLP Acuratete pe testare logistic adam 0.001", accuracy(predictions, labels[indices_test]))
print("Matricea de confuzie")
print(confusion_matrix(labels[indices_test], predictions))

# # 10 fold cross validation
# scores = cross_val_score(mlp, all_data_train_bow, labels, cv=10)
# print("Accuracy 10 fold cross validation: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))